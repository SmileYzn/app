            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="g-2 d-flex gap-2">
                        ...
                        <?php if(in_array('editar', $_BACKEND['acesso'])): ?>
                        <a class="btn btn-app btn-warning btn-trash" href="#" data-bs-toggle="modal" data-bs-target="#modal-alterar">
                ...
                <form method="POST" class="card-body table-responsive p-0">
                    <table class="table table-striped table-hover align-middle text-nowrap text-center m-0">
                        ...
                        foreach($_DADOS as $row)
                        {
                            ...
                            <tr>
                                ...
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= $row['ordem'] ?></a></td>
                                ...
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= ($row['ocultar'] ? '<i class="bi-eye-slash-fill text-danger" title="Ocultar"></i>' : '<i class="bi-eye-fill text-success" title="Exibir"></i>') ?></a></td>
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><i class="<?= $row['icone'] ?> text-primary-emphasis" title="<?= $row['icone'] ?>"></i></a></td>
                            </tr>
                            <?php
                        }
                        ?>
                    ...
                    <?php include_once("{$_ENV['SG_PATH_PUBLIC']}modal/alterar.php"); ?>
                </form>
