                ...
                <div class="card-body">
                    <div class="row g-3">
                            ...
                            <select class="form-select" id="moduloFK" name="moduloFK" required>
                                ...
                                foreach ($mods as $mod)
                                {
                                    ...
                                    <optgroup label="<?= $mod['ordem'] ?>. <?= $mod['nome'] ?>">
                                        <option value="<?= $mod['id'] ?>" <?= $_POST['moduloFK'] == $mod['id'] ? 'selected' : '' ?>><?= $mod['ordem'] ?>. <?= $mod['nome'] ?></option>
                                        <?php
                                        $subs = (new Modulo)->consultarAtivo("moduloFK = {$mod['id']}", "ORDER BY ordem");
                                        foreach ($subs as $sub)
                                        {
                                            ?>
                                            <option value="<?= $sub['id'] ?>" <?= $_POST['moduloFK'] == $sub['id'] ? 'selected' : '' ?>><?= $sub['ordem'] ?>. <?= $sub['nome'] ?></option>
                                            <?php
                                        }
                                        ?>
                                    </optgroup>
                        ...
                        <div class="col-md-4">
                            ...
                            <select class="form-select" id="acesso" name="acesso[]" required multiple>
