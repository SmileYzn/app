            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="g-2 d-flex gap-2">
                        ...
                        <a class="btn btn-app btn-warning btn-trash" href="#" data-bs-toggle="modal" data-bs-target="#modal-alterar-acesso">
                ...
                <form method="POST" class="card-body table-responsive p-0">
                    <table class="table table-striped table-hover align-middle text-nowrap text-center m-0">
                        ...
                        {
                            $modulo = (new Modulo)->id($row['moduloFK']);
                            $acesso = (new Acesso)->consultarAtivo("LOCATE(acesso, '{$row['acesso']}')");
                            ?>
                            <tr>
                                <td><input class="form-check-input" type="checkbox" name="id[<?= $row['id'] ?>]" value="<?= $row['id'] ?>"></td>
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= $modulo['nome'] ?></a></td>
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= $modulo['url'] ?></a></td>
                                <td>
                                    <?php foreach($acesso as $ac): ?>
                                    <i class="<?= $ac['icone'] ?> text-<?= $ac['cor'] ?>" title="<?= $ac['nome'] ?>"></i>
                                    <?php endforeach; ?>
                                </td>
                            </tr>
                            <?php
                    ...
                    <?php include_once("{$_ENV['SG_PATH_PUBLIC']}modal/alterar-permissao.php"); ?>
                </form>
