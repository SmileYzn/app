            ...
            <div class="card">
                ...
                <form method="POST" action="<?= $_SERVER['PHP_SELF'] ?>" class="card-body collapse <?= !empty($_SESSION[$_SERVER['SCRIPT_NAME']]) ? 'show' : '' ?>" id="collapse-pesquisa">
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <select class="form-select" id="areaFK" name="areaFK">
                                <option value="" selected>-- TODAS --</option>
                                <?php foreach ((new Area)->consultarAtivo("id >= " . Sessao::get('areaFK'), 'ORDER BY nome') as $row): ?>
                                <option value="<?= $row['id'] ?>" <?= $_SESSION[$_SERVER['SCRIPT_NAME']]['areaFK'] == $row['id'] ? 'selected' : '' ?>><?= $row['nome'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-2">
                ...
                <form method="POST" class="card-body table-responsive p-0">
                    <table class="table table-striped table-hover align-middle text-nowrap text-center m-0">
                        ...
                        {
                            $unidade  = (new Unidade)->id($row['unidadeFK']);
                            $situacao = (new Situacao)->id($row['situacaoFK']);
                            $area     = (new Area)->id($row['areaFK']);
                            ?>
                            <tr>
                                ...
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= (!empty($row['unidadeFK']) ? $unidade['nome'] : 'Todas') ?></a></td>
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><?= $area['nome'] ?></a></td>
                                <td><a href="<?= $_BACKEND['caminho'] ?>/editar.php?id=<?= Extra::enc($row['id']) ?>"><span class="badge <?= ($situacao['id'] == '1' ? 'bg-success' : 'bg-warning') ?>"><?= $situacao['nome'] ?></span></a></td>
                            </tr>
                            <?php
                        }
                        ?>
